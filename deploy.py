...
def install_depends_by_apt(c):
    c.local(
        'apt install unzip python3.7-dev fonts-noto fonts-noto-color-emoji chromium-browser nginx redis npm git python-pip -y')
...
def install_driver(c):
    ...
    c.local('mv -f chromedriver /usr/local/share/chromedriver')
    c.local('ln -s /usr/local/share/chromedriver /usr/local/bin/chromedriver')
    c.local('ln -s /usr/local/share/chromedriver /usr/bin/chromedriver')
